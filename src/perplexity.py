...
def calculate_ppl(
    ...
    for glottocode in glottocodes:
        ...
        ppl_per_language[glottocode] = {
            "ppl_per_token": torch.exp(torch.tensor(loss)).item() if tokens_per_language[glottocode] > 0 else float('inf'),
            "ppl_per_morpheme": torch.exp(torch.tensor(loss * (tokens_per_language[glottocode] / morphemes_per_language[glottocode]))).item() if morphemes_per_language[glottocode] > 0 else float('inf'),
            "ppl_per_word": torch.exp(torch.tensor(loss * (tokens_per_language[glottocode] / words_per_language[glottocode]))).item() if words_per_language[glottocode] > 0 else float('inf'),
        }
