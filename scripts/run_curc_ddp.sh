...
if [[ "$2" == "--monoling" ]]; then
    ...
    do
        ...
        torchrun \
            ...
            run.py "$1" --overrides glottocode=$glottocode models_dir=/work/hdd/bebe/$USER/polygloss/ batch_size=32
    done
else
    torchrun \
    --nproc_per_node=$SLURM_NTASKS_PER_NODE \
    ...
    --master_addr=$MASTER_ADDR \
    --master_port=$MASTER_PORT \
    run.py "$1" --overrides models_dir=/work/hdd/bebe/$USER/polygloss/ batch_size=32
fi
