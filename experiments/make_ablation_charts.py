...
# Ablation 2
f, ax = plt.subplots(figsize=(4, 1.4))
...
# Ablation 4
f, ax = plt.subplots(figsize=(4, 2))
data = df[df["Method"].isin(["PolyGloss [interleaved]", "PolyGloss [qwen]"])]
data["Method"] = data["Method"].replace(
    {
        "PolyGloss [interleaved]": "ByT5",
        "PolyGloss [qwen]": "Qwen",
        "PolyGloss [aya]": "Aya Expanse",
    }
)
ax.tick_params(
    axis="x",
...
# GRPO
f, ax = plt.subplots(figsize=(4, 1.4))
data = df[df["Method"].isin(["PolyGloss [concat]", "GRPO"])]
data = data[data["Language"] == "git"]
data["Method"] = data["Method"].replace(
    {"PolyGloss [concat]": "PolyGloss", "GRPO": "+ GRPO"}
)
# summary = df.groupby(["Method", "Metric"], as_index=False).agg(
#     Mean=("Score", "mean"),
#     sem=("Score", "sem"),
...
for text in ax.legend_.get_texts():
    text.set_fontweight("bold")
plt.savefig(
    "experiments/viz/grpo.pdf", format="pdf", bbox_inches="tight", pad_inches=0.01
)
plt.show()
plt.close(f)
